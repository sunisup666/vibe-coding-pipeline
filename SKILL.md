...
> **时机:** 最迟在第 4 步开发切片文档生成前完成骨架拉取,否则 Slice 1 无法定义项目结构。
...
4. **自检不是自我背书**:每条检查必须引用文档里的原文/具体位置作为通过的依据(如"§5.1 已写明 [] 不覆盖非空"),不能只写"已确认""无问题"。任一条不满足,就地改文档再重跑自检,而不是在自检说明里声称通过。跨文档一致性类的检查(依赖三处一致、字段命名、状态/枚举对齐、fixture 命名),要真的把两处拉出来逐一比对,不能凭印象。
5. **呈现 + 指出重点审查项**:主动告诉用户这一步该重点看哪(见"门禁"),别只说"你看看"。
6. **审阅迭代**:满意 → 落盘 `docs/` 并**更新 `docs/_progress.md`**;不满意 → 记录反馈,修改再呈现,顺着反馈一点点改到满意。不要一次问一堆。
7. **提示词复盘与回流**:定稿后按 `references/prompt-tuning.md` 复盘,只把**可泛化缺陷**改进进本步提示词,给用户看 diff,确认后写回本 skill 的 `references/prompts/`。项目特有内容绝不进提示词。拿不准就问,或先记进进度文件的"待确认回流",留到之后确认。
8. **收尾**:更新进度文件,简述"这步完成了什么、下一步做什么、需要用户先准备什么",再继续。
